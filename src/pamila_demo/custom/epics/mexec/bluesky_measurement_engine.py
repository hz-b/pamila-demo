...
from bact_bessyii_mls_ophyd.devices.utils.multiplexer_for_settable_devices import MultiplexerProxy
...
def setup(device_ids: Sequence[str], prefix="Anonym:"):
    ...
    quad_pcs = {
        name: PowerConverter(f"{prefix}{name}:", name=name, readback_suffix="rdbk",
                                     setpoint_suffix="set")
    ...
    vert_steerer_pcs = {
        name.lower(): PowerConverter(f"{prefix}{name}:", name=name.lower(), readback_suffix="rdbk",
                                     setpoint_suffix="set")
    ...
    sextupoles = MultiplexerProxy(name="sext_col", settable_devices=sext_pcs, default_name=list(sext_pcs)[0])
    steerers = MultiplexerProxy(
        name="st_col", settable_devices=steerer_pcs, default_name=list(steerer_pcs)[0],
