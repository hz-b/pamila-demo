...
def orm(*, horizontal_steerer_names: Sequence[str], vertical_steerer_names: Sequence[str],
    ...
    lt = LiveTable([sig.name for _, sig in info_sigs.items()] + [orbit.count.name],  # + list(actuators.values())
                   default_prec=10, )
    ...
    uid = mexec.execute(commands_collection=cmds_on_machine.commands,  # need to add bpms
                        detectors=[orbit], actuators=actuators, info_signals=info_sigs, md=md, )
